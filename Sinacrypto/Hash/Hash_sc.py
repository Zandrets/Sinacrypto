...
def MD5_sc(data):
    ...
    open(os.path.dirname(os.path.dirname(__file__))+'/'+'encrypt_system.log', 'at').write(log)
...
def MD5_pcd(data):
    ...
    size=len(data)
    log="starting MD5 encode of %d bytes at %s\n" % (size, log_start_time)
    open(os.path.dirname(os.path.dirname(__file__))+'/keys/'+'encrypt_system.log', 'at').write(log)
    return MD5.new(data)